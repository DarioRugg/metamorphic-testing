...
def main(cfg : DictConfig) -> None:
    assert not cfg.bias, "bias must be false when training the classifier"
    ...
    for k in range(cfg.cross_validation.folds) if cfg.cross_validation.flag else [0]:
        ...
        lightning_model = LitClassifier(cfg, dataset.get_num_features(), ae_checkpoint_path=cfg.model.ae_weights)

        clf_trainer = Trainer(max_epochs=cfg.clf_model.epochs, callbacks=callbacks,
        ...
        clf_trainer.fit(lightning_model, datamodule=dataset)
        ...
        test_y_hat = torch.cat(clf_trainer.predict(lightning_model, datamodule=dataset), dim=0)
